...
extension UIViewController {
    ...
    func showMessageWithInput(title title: String, message: String, textFieldPlaceholder: String, confirmTitle: String, declineTitle: String, viewController: UIViewController?) {
        
        let alertController = UIAlertController(title: title, message: message, preferredStyle: .Alert)
        alertController.addTextFieldWithConfigurationHandler { (textField) -> Void in
            textField.placeholder = textFieldPlaceholder
        }
        let cancelAction = UIAlertAction(title: declineTitle, style: .Cancel, handler: nil)
        
        let OKAction = UIAlertAction(title: confirmTitle, style: .Default) { (action) -> Void in
            let inputText = ((alertController.textFields?.first)! as UITextField).text!
            
            if viewController is AddPostViewController {
                let vc = AddPostViewController()
                vc.nameFormSubmitted(name: inputText)
            }
        }
